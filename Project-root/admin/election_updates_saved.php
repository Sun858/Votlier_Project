...
try {
    ...
    $stmt->bind_param("ssssi", $election_type, $election_name, $start_datetime, $end_datetime, $poll_id);
    if (!$stmt->execute()) {
        throw new Exception("Error updating election: " . $stmt->error);
    }
    $stmt->close();
    ...
    if (!$stmt->execute()) {
        throw new Exception("Error clearing old candidates: " . $stmt->error);
    }
    $stmt->close();
