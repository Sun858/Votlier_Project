...
/*Same function again, but for logging a new vote. Maybe this really isn't necessary, and probably having repeated functions isn't good.
I can probably make it dynamic? Maybe?
*/
function recordVotingAttemptDB($conn, $ip, $resource) {
    $stmt = $conn->prepare("INSERT INTO login_attempts (ip_address, resource, attempt_time) VALUES (?, ?, NOW())");
    $stmt->bind_param("ss", $ip, $resource);
    $stmt->execute();
    $stmt->close();
